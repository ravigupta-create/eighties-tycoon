      ...
      <div className="border border-crt-border rounded p-4">
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm mb-3">
          ...
          <p className="text-amber">
            <span className="text-amber-dim">AGE: </span>{state.age}
          </p>
          <p className="text-amber">
        ...
        {log.length === 0 ? (
          ...
          log.map((entry) => (
            <div key={entry.id} className="mb-1.5">
              <span className="text-phosphor-dim text-[10px]">
                {entry.monthName.slice(0, 3)} {entry.year}
              </span>
              <p className={`text-xs leading-tight ${colorFor(entry.type)}`}>
                {'>'} {entry.text}
              </p>
            </div>
          ))
          ...
          <div className="crt-screen bg-crt-dark border-2 border-crt-border rounded-lg shadow-[0_0_30px_rgba(51,255,51,0.1)]">
            ...
            <div className="p-3 border-b border-crt-border bg-crt-bg">
              <button
                ...
                className="w-full bg-crt-bg border-2 border-phosphor text-phosphor font-[family-name:var(--font-crt)] text-base px-4 py-3 rounded cursor-pointer hover:bg-phosphor hover:text-crt-bg transition-all text-glow font-bold tracking-wider"
              >
                {'>>>'} NEXT MONTH {'>>>'}
              </button>
            </div>
